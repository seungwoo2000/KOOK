...
def _has_broth(menu, ingredients):
    # 실측: 이름 문자열 매칭만으론 틀림 — 육개장은 실제 F.MENU_CLASS='국'인데 이름에
...
# 본 육수량 계산 — LLM에 "5~8배로 판단해서 써라"고 맡기면 계산을 하지 않고 "국 한 그릇은 보통
# 500~600ml"라는 감으로 찍는다(실측: 유부장국 건더기 16g이면 80~128ml여야 하는데 600ml가 나옴).
# 그래서 코드가 직접 계산해 고정값으로 못박고, LLM은 그 값을 그대로 쓰기만 한다.
BROTH_WATER_MULTIPLIER = 6   # 건더기 총중량의 배수(5~8배 범위의 중간값)
BROTH_WATER_MIN = 300        # ml, 1인분이라도 국 한 그릇 답으려면 이 정도는 있어야 함
...
# 조리 단계 안의 알맞은 위치에 반영한다 — 접히는 안내만으로는 읽지 않고 넘어가 순응도가 떨어진다.
# 고칼륨 채소가 아닌 재료(양파·파·마늘 등)는 대상에서 자동으로 빠진다.
...
def _fmt_amt(a):
    ...
    전부 :.0f로 반올림하면 후추 0.2g·마늘 0.3g 같은 소량 재료가 "0g"이 되어, 재료가 아예
    안 쓰인 것처럼 LLM에 전달된다.
    0.05g처럼 소수점 1자리로도 0이 되는 극소량(아주 작게 스케일한 저염오이김치의 소금
    0.025g 등)은 같은 이유로 0.1g을 바닥으로 둔다."""
    if a > 0:
        r = round(a, 1) or 0.1
    else:
        r = 0
...
def edit_recipe(menu, ingredients, model="gpt-4o-mini", source_menu=None):
    ...
    재시도 결과도 놓치면 그냥 그대로 반환(무한 재시도 안 함)."""
