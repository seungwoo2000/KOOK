...
# 그 끼는 줄일 대상이 없어 열량 초과를 못 잡으므로, load_all()에서 Class 기준으로 자동 확장한다.
...
# 나트륨 기여가 실질적인 조미료만 축소 대상으로 삼는다. 식초·후추처럼 나트륨이 거의 없는
# 조미료까지 비율로 같이 줄이면 맛만 잃고 목표치에는 기여가 없다.
...
def load_all():
    ...
    # 저칼륨 과일 간식(임상 권장 1회 섭취량). raw데이터의 단감(100g)·딸기(150g)는 일반 인구
    # 기준 분량이라 칼륨 임계치를 넘으므로, 여기서 안전 분량으로 덮어쓴다(raw 원본은 안 건드림).
    for name, ings in EXTRA_FRUIT_SNACK_RECIPES.items():
        master[name] = [(ing, str(amt)) for ing, amt in ings]
        MENU_CLASS[name] = '과일(간식)'
    # 원본 raw데이터에서 다수결 병합이 틀린 값을 고르는 경우를 바로잡는다.
    # 너비아니구이는 7개월치에 "미트볼, 냉동"으로 입력돼 있어(조리법은 "너비아니를 올려 굽는다"인데
    # 재료만 미트볼) 다수결로는 그쪽이 뽑힌다. 진짜 너비아니(소고기) 레시피는 3·9·12월에만 있고,
    # 그중 9월판(얇게 썰어 두드리고 배즙양념 — 정통 방식)을 쓴다.
    for name, ings in RECIPE_FIX_OVERRIDE.items():
        master[name] = [(ing, str(amt)) for ing, amt in ings]
    # 참깨·들깨·견과류·건포도는 요리 재료에서 제거.
    ...
    for mn in menus:
        ...
        for ing, amt in master.get(mn, []):
            ...
            if is_broth(ing, a):                                 # 육수용 재료 → 육수 전면 금지, 재료 자체를 제외
                # '물'로 치환하지 않고 아예 뺀다. 육수용 재료의 무게(멸치3g+다시마1.5g)는 "국물
                # 만들 물의 양"이 아니라서, 물로 바꾸면 재료표에 "물 4.5g"처럼 국 한 그릇답지 않은
                # 뜻모를 숫자가 남는다.
                # 원본 데이터 자체가 국물 물의 양을 재료·조리과정 어디에도 기록 안 해서
...
# 인 흡수율 보정 (임상영양사 지정): 식물성 유기인(피테이트)은 흡수율이 낮아 ×0.7,
...
#  타겟으로 골라지면 칼륨 개선 명목으로 나트륨을 대량 주입하게 됨.
...
# 대체 채택시 식품군 호환 판정: 대체사전(apply_potassium/phosphorus_filter.py)이
...
CLINICAL_K_SWAP = {
    ...
    '참외':         '사과, 부사, 생것',
    # '대두':'두부' 제거: 원물 검은콩·서리태를 겨냥해 만들었으나
...
# (2)'성인' 계열: 대상자가 전부 성인이라 '성인'이 붙은 큰 분량판을 표준으로 쓰되
...
MENU_DISPLAY_RENAME = {
    ...
    '채소달걀말이성인': '채소달걀말이',
    '토마토달걀볶음성인': '토마토달걀볶음',
    # 참깨·들깨·견과류 제거 후 이름-내용 불일치 정리. 병합은 안 되고(레시피가 실제로
...
def rename_menu_for_swap(inst, menu, old_ing, new_ing):
    ...
    # 토란대나물→무나물처럼 원래도 자연스러운 경우는 영향 없음.
...
# 인도 동일 원칙(K와 대칭). 문턱이 없으면 g>0이면 무조건 교체돼 효과 없는 교체가 쌓인다.
# 실측 최소사례(부대찌개 소시지7.5g→닭고기)가 gain 4.58mg로 이 문턱(5.0) 바로 아래라 걸러지고,
# 나머지 실사용(대부분 25g+)은 영향 없다. 사전이 확장될 때를 대비한 안전장치이기도 하다.
P_SWAP_MIN_GAIN = 5.0

# 향신채(마늘·생강)는 소량으로 맛의 베이스를 결정하므로 일반 재료보다 높은 절감기준 적용.
...
def is_sole_solid_ingredient(pool, menu, exclude):
    ...
    (실측: 감자/고구마 90여개 메뉴 중 이 패턴은 단 4개뿐이었음)"""
...
def _best_k_swap(pool, protect_identity=False):
    ...
    for i in pool:
        ...
        # 고구마줄기·토란대(줄기 부위)도 허용: 나물↔나물 대체 자체는 문제없음
        ...
        # '삶은무'·'찐무'·'무맛탕'·'오이쌈쌈장'은 존재하지 않는 요리라 대체보다 양조절이 맞음.
...
def lever_potassium(inst, kmax, anchor=None):
    ...
    순서: (1)비앵커 교체 (2)비앵커 양 감소(인 레버와 대칭 — 임상 안전성을 위해 비앵커도 교체
    실패시 포기하지 않고 양조절로 폴백) (3)앵커 교체 (4)그래도 없으면 앵커 메뉴 전체를 비율
    맞춰 통째로 축소(scale_menu, 나트륨 레버와 동일 원칙) — 정체성 재료만 줄이면 마늘·참기름 등
    나머지 양념 비율이 깨지므로 메뉴 전체를 같은 비율로. 하한 50%.
    ...
    for _ in range(25):
        ...
        for i in non:
            ...
            if is_sole_solid_ingredient(non, i['menu'], exclude=i):
                continue   # 유일한 고형재료면 대체 대신 양조절로(K레버와 동일 원칙)
        ...
        # factor 0.8→0.7: 가공육 1.5가중으로 드러난 위반을 양감소가 더 빨리 흡수.
...
def lever_phosphorus_rawP(inst, pmax, anchor=None, plo=0):
    """lever_phosphorus의 raw P 기준 버전(두부·콩류 앵커 전용 조건부 경로).
...
PROTEIN_CALORIE_CAP_FRAC = 0.90   # 두부·콩류 앵커 전용 raw P 예산 cap 배율.
# protein_phosphorus_final_compare/cross_anchor 실험에서 검증된 값. 0.80/1.00은 세 앵커 어디서도
# 필요성이 확인되지 않아 0.90을 쓴다.
...
def lever_protein_capped(inst, lo, hi, anchor=None, pmax=None):
    """lever_protein의 raw P 예산 cap 버전(두부·콩류 앵커 전용 조건부 경로).
...
# 저칼륨 과일 간식 — 임상 권장 1회 섭취량 기준. 기존 raw데이터의
...
def add_snack(inst, need_kcal, kmax=None, pmax=None):
    ...
    (마지막 레버 패스에서 호출되므로 이후 검증 기회가 없음 — 여기서 직접 걸러야 함)."""
...
OIL_ADD_MAX_G = 10.0  # 새로 추가하는 참기름은 요리 하나당 이 이하로(넉넉한 나물 수준, 88kcal 내외).
                       # 5g로 조이면 열량 통과율이 19포인트 떨어져 10g으로 둔다.
VINEGAR_KW = ('식초',)  # 초무침·초절임류는 신맛에 참기름의 고소함이 겹치면 텁텁해져 제외


def add_oil(inst, need_kcal):
    """열량 부족분을 기름으로 보충. '열량보충(기름)' 같은 별도 항목으로 덜렁 붙이지 않고
    (1) 이미 기름을 쓰는 요리의 기름을 늘리거나 (2) 나물·무침 같은 요리에 참기름을 더한다.
    5kcal 미만은 무시한다 — 안 그러면 0.0g짜리 빈 항목이 네 끼 중 한 끼꼴로 붙는다.
...
def lever_calorie(inst, lo, hi, anchor=None, allow_snack=True, kmax=None, pmax=None):
    ...
    생기므로, 마지막 패스에서만 True로 호출."""
    e = totals(inst)['E']
    if e > hi:
        # 초과분 축소 순서: 밥(주식) 먼저 → 하한(40%)까지 다 써도 남으면 기름.
        # 밥과 기름을 한 풀로 묶어 동시에 깎으면 우선순위가 사라진다.
        ...
        if e2 < lo:
            ...
            # 시도하고, 간식은 기름으로도 못 메운 진짜 마지막 순간에만 최소한으로 투입.
            add_oil(inst, lo - e2)
            e3 = totals(inst)['E']
...
def lever_calorie_capped(inst, lo, hi, anchor=None, allow_snack=True, kmax=None, pmax=None):
    """lever_calorie의 raw P 예산 cap 버전(두부·콩류 앵커 전용 조건부 경로).
...
def _plant_protein_path_needed(inst, menus, anchor):
    """주찬(menus[2]) 또는 지정앵커가 두부·콩류 식품군인지 판정.
...
def adjust(menus, b, anchor=None):
    ...
    p_ok = True
    # 두부·콩류 앵커(주찬 또는 지정메뉴) 전용 raw P 통일 경로(조건부 반영).
    ...
    for pass_i in range(2):
        ...
        # 칼륨/인 레버가 그 K/P를 보고 되먹임(다른 메뉴 깎기)을 일으킴.
        if use_rawP_path:
